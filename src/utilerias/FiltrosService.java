...
class FiltrosService {
	...
	Mat gaussianBlur(Mat src, String kernel, String sigmaX) {
		Mat dst = new Mat(), dst2 = new Mat();
		Imgproc.cvtColor(src, dst, Imgproc.COLOR_BGR2GRAY);
	...
	Mat escalaGrises(Mat src) {
		Mat dst = new Mat();
		Imgproc.cvtColor(src, dst, Imgproc.COLOR_BGR2GRAY);
		return dst;
	}

	Mat bilateralFilter(Mat src, String delta, String sigmaColor, String sigmaSpace) {
	...
	Mat medianBlur(Mat src, String kernel) {
		Mat source = src.clone(), dst = new Mat();
